...
async function serialize(msg, conn) {
 ...
 conn.sendFileUrl = async (recipientId, fileUrl, caption = '', quotedMessageId = '', options = { author: 'Asta-Md' }, fileTypeOverride = '') => {
  const response = await axios.head(fileUrl)
  const contentType = response?.headers['content-type'] || ''
  const mainType = contentType.split('/')[0]
  let messageOptions = false
  ...
  } else if (mainType === 'image' || fileTypeOverride === 'image') {
   messageOptions = {
    image: { url: fileUrl },
    caption,
    ...options,
  ...
  } else if (mainType === 'video' || fileTypeOverride === 'video') {
   messageOptions = {
    video: { url: fileUrl },
    caption,
    mimetype: 'video/mp4',
  ...
  } else if (mainType === 'audio' || fileTypeOverride === 'audio') {
   messageOptions = {
    audio: { url: fileUrl },
    mimetype: 'audio/mpeg',
    ...options,
  ...
  if (messageOptions) {
   try {
    return await conn.sendMessage(recipientId, messageOptions, { quoted: quotedMessageId })
   } catch (error) {}
  }

  try {
   const contentDisposition = response?.headers['content-disposition']
   const fileName = contentDisposition?.split('="')[1]?.split('"')[0] || 'file'
   ...
   return await conn.sendMessage(recipientId, { document: { url: fileUrl }, ...fileOptions }, { quoted: quotedMessageId })
  } catch (error) {}
  ...
  return await conn.sendMessage(recipientId, { document: { url: fileUrl }, ...fallbackOptions }, { quoted: quotedMessageId })
 }
 conn.sendFromUrl = conn.sendFileUrl
 conn.downloadAndSaveMediaMessage = async (message, fileName = 'null', returnBuffer = false, saveToFile = true) => {
  const mediaMessage = message.msg || message
  const mimeType = mediaMessage.mimetype || ''
  const mediaType = message.mtype ? message.mtype.split(/Message/gi)[0] : mediaMessage.mtype ? mediaMessage.mtype.split(/Message/gi)[0] : mimeType.split('/')[0]
  ...
  const filePath = `./temp/${fileName}.${fileType.ext}`
  fs.writeFileSync(filePath, mediaBuffer)

  return filePath
 }
 conn.sendTextWithMentions = async (chatId, text, quotedMessage, options = {}) => {
  const mentionedJids = [...text.matchAll(/@(\d{0,16})/g)].map((match) => `${match[1]}@s.whatsapp.net`)
