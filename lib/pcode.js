'use strict'
const { default: makeWASocket, useMultiFileAuthState, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, DisconnectReason, delay, generateSessionID } = require('baileys')
 ...
 async function connect() {
  ...
  sock.ev.process(async (events) => {
   if (events['connection.update']) {
    ...
    if (connection === 'close' && (status !== 403 || status !== 401 || !status)) {
    ...
    if (connection === 'open') {
     ...
     await delay(5000)
     console.log("session generated using pairing code run 'npm start' to start the bot")
     process.exit(0)
    }
   }
  })
