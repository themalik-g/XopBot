...
async function generateContent(prompt, imageBuff) {
 ...
 const result = await model.generateContent([prompt, fileToGenerativePart(imageBuff, imageBuff && (await fromBuffer(imageBuff)).mime)])

 return result.response.text()
}

async function gemini(prompt, imageBuff, options) {
 const { promptText, promptImage } = await getJson(`https://gist.github.com/Neeraj-x0/d80f8454b0f1c396a722b12cd159945e/raw`)
