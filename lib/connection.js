...
const { default: makeWASocket, useMultiFileAuthState, fetchLatestBaileysVersion, Browsers, delay, makeCacheableSignalKeyStore, DisconnectReason, makeInMemoryStore, signedKeyPair } = require('baileys')
...
const connect = async () => {
 ...
 process.on('unhandledRejection', async (err) => {
  ...
  const text = `\`\`\`Zenon-bot unhandledRejection: \n${error}\`\`\``
  await conn.sendMessage(conn.user.id, { text })
  console.log(err)
 })
 process.on('uncaughtException', async (err) => {
  const error = util.format(err)
  const text = `\`\`\`Zenon-bot uncaughtException: \n${error}\`\`\``
  await conn.sendMessage(conn.user.id, { text })
  console.log(err)
 })
 return conn
...
const handleConnectionUpdate = (conn) => async (s) => {
 ...
 if (connection === 'connecting') console.log('Connecting to WhatsApp... Please Wait.')
  ...
  if (/true/gi.test(config.SESSION_FLUSH) || !conn.authState.creds?.myAppStateKeyId) {
   console.log('REBUILDING SESSION_ID' || conn.authState.creds?.myAppStateKeyId)
   conn.ev.flush()
  }
